...
- 「ゃ・ゅ・ょ」などの小書き文字に対応しています。例えば「しやくしょ → しょくいん」は正しいつながりとして判定します。
...
- [Deno Docs: Writing an HTTP Server](https://docs.deno.com/runtime/fundamentals/http_server/)
  - `Deno.serve()` による HTTP サーバーと `serveDir()` による静的ファイル配信の実装で参考にしました。
...
- **全般的なデバッグ**
  ...
  - `deno check` による構文確認と、代表的な入力例を使った動作確認に使用しました。

AI が出力したコードはそのまま採用せず、実際の辞書データと入力例を用いて動作を確認しました。
